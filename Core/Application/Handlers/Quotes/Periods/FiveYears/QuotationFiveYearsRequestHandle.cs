...
public class QuotationFiveYearsRequestHandle : BaseQuotationRequestHandle
{
	public QuotationFiveYearsRequestHandle() { }

	public string QuotationFiveYearsRequest(string ticker, [Optional] string frequency)
	{
        UnixDateTime _parser = new UnixDateTime();
        string start = _parser.ToDateUnix(DateTime.Now.AddYears(-5).Date);
